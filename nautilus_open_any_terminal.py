...
def get_directory_menu_items(
    ...
    *,
    ...
    if remote:
        if foreground:
            REMOTE_LABEL = _("Открыть {} удалённо")
            REMOTE_TIP = _("Открыть {} удалённо в {}")
            LOCAL_LABEL = _("Открыть {} локально")
            LOCAL_TIP = _("Открыть {} локально в {}")
            tip = REMOTE_TIP.format(terminal_name, file.get_name())
        else:
            REMOTE_LABEL = _("Открыть {} удалённо")
            REMOTE_TIP = _("Открыть {} удалённо")
            LOCAL_LABEL = _("Открыть {} локально")
            LOCAL_TIP = _("Открыть {} локально")
...
def set_terminal_args(*_args):
    ...
    print(
        f'open-any-terminal: terminal is set to "{terminal}" {new_tab_text} {flatpak_text}'
    )
